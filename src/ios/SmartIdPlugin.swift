...
@objc(SmartIdPlugin) class SmartIdPlugin: CDVPlugin {
  ...
  func initSmartId(command: CDVInvokedUrlCommand) {
    ...
    var pluginResult = CDVPluginResult(
      status: CDVCommandStatus_OK, messageAs: "The plugin succeeded")
  ...
  func smartCoreOperation(command: CDVInvokedUrlCommand) {
    ...
    let newAccount = Account(
      client: beer.account?.client ?? "", clientRefId: 0, clientRefIdStr: "", email: "",
      phoneNumber: "", session: "", accountNumber: "", bank: "")
